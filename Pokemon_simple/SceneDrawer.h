...
public:
    SceneDrawer(){};
    void draw(GameSence gs, int money = 0, Map* map = nullptr, Log* log = nullptr, Combat* combat = nullptr, Pokemon* pokemon=nullptr);    //根据游戏状态绘制场景, 只绘制固定不变的部分, 会随着选择而变化的部分由Command类绘制
private:
    ...
    void drawPokemonCenter(Pokemon *pokemon); //显示"宝可梦中心", 和携带的宝可梦信息
